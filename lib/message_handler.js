  ...
  function replyToUser(username, { name, is_group, is_channel }) {
    ...
    return (text) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (is_group) {
          traductor.postMessageToGroup(name, text, { as_user: process.env.BOT_AS_USER }, resolve);
        } else if (is_channel) {
          traductor.postMessageToChannel(name, text, { as_user: process.env.BOT_AS_USER }, resolve);
        } else {
